...
## ADR-14 — Composite Transaction index created, not applied, pending owner sign-off
- **Decision:** `prisma/schema.prisma`'s `Transaction` model gains `@@index([positionId, executedAt])` — the access pattern for the closed-positions FIFO replay (ADR-5). The migration (`prisma/migrations/20260718152229_add_transaction_position_executedat_index`) was generated via `prisma migrate dev --create-only` and is **not applied** to the shared dev/prod database.
- **Evidence:** `prisma/schema.prisma:86-89`, `prisma/migrations/20260718152229_add_transaction_position_executedat_index/migration.sql` (single additive `CREATE INDEX`, no destructive statements). `prisma migrate status` confirms the migration is pending, not applied. `plans/2026-07-18-performance-audit-remediation.md` Task 6.
- **Rationale:** dev and production share one database (ADR-6/TD-02) — there is no isolated environment to apply and test a migration against before it hits real data. The plan's Open decisions section requires explicit owner sign-off before `prisma migrate deploy` runs. A `@@index([type])` addition (also mentioned in the plan as conditional on query-plan benefit) was not pursued — no isolated DB exists to safely run `EXPLAIN` against, so that decision is deferred rather than guessed at.
- **Tradeoffs:** the closed-positions hot path (ADR-5: "recomputes from scratch on every request... cost grows with transaction history size") does not yet benefit from the index until it's applied. Accepted because applying a schema migration to a shared production database without owner sign-off is a bigger risk than a temporarily-missing index.
- **Status:** proposed — blocked on owner sign-off to apply
- **Confidence:** High (the SQL itself is safe and additive); the timing of application is entirely an owner decision, not a technical one.
