...
- `User` 1:1 `UserScoringPreferences` (`plans/2026-07-20-configurable-scoring-weights.md` + `plans/2026-07-21-scoring-weights-direct-percent.md`, ADR-20/21/22) — stores per-user composite category weights and fundamental subcategory weights as ten discrete nullable `Float` columns. As of ADR-22, the columns hold **whole-percent values summing to 100 (validated at write time by `saveWeights`)**, normalized at scoring time by `lib/utils/scoring-weights.ts` — scale-invariant, so scoring is unchanged (a group stored as percents `{25,25,20,15,15}` normalizes to the identical fractions as the pre-ADR-22 raw-relative-number storage `{0.25,...}` and produces byte-identical scores). Two accessors deliberately diverge in shape: `getWeights(userId)` (raw-coalesced numbers, unchanged, used by the three scoring consumers below) and `getWeightsForSettings(userId)` (normalizes then converts to whole percents, used only by the settings route's GET). The composite score is reweighted client-side in `components/overview.tsx` (and identically in `lib/services/wishlist.service.ts`, the second consumer — both call the same shared `weightedCompositeTotal`); the fundamental score is reweighted server-side by passing the user's weights into `fetchFundamentals(symbol, weights?)` — the on-read reweight is never written back, so `FundamentalData` stays user-independent (ADR-4/ADR-21). **The migration (`prisma/migrations/20260720220007_user_scoring_preferences`) is additive-only and was generated via `prisma migrate dev --create-only`, then applied to the shared dev/prod database on 2026-07-21** (`prisma migrate status` confirms "Database schema is up to date!" — no pending migrations). `GET`/`PUT /api/settings/scoring-weights` and the per-user reweight paths are live against the real database. ADR-22 adds no schema/migration change — only the stored number's meaning/scale, in code.
...
| `lib/utils/scoring-weights.ts` | Pure `DEFAULT_SCORING_WEIGHTS` + `normalizeWeights`/`normalizeCompositeWeights`/`normalizeFundamentalWeights`/`weightedCompositeTotal`/`weightedFundamentalTotal`/`weightsEqualDefaults` (`plans/2026-07-20-configurable-scoring-weights.md`, ADR-20/21) — the single source of truth for scoring-weight math, imported by the client composite (`components/overview.tsx`), the wishlist composite (`lib/services/wishlist.service.ts`), the server fundamental path (`lib/services/fundamental-analysis.service.ts`, both its per-user reweight and its own default-weighted total), and the two Headline-score-card "Your weighting" meta-kicker comparisons (`overview.tsx`, `fundamental-analysis.tsx`). As of ADR-22 (`plans/2026-07-21-scoring-weights-direct-percent.md`) also exports `fractionsToPercents`/`percentsToFractions`/`sumsTo100` — pure conversion/validation helpers at the settings API boundary only; they do not change the internal fraction-based scoring scale or any of the functions above |
| `lib/services/scoring-preferences.service.ts` | `getWeights`/`saveWeights` — reads/writes `UserScoringPreferences`, coalescing null columns (and a missing row) to `DEFAULT_SCORING_WEIGHTS`; consumed by `app/api/settings/scoring-weights/route.ts` (GET/PUT) and directly by `app/api/market/fundamentals/[symbol]/route.ts` and `wishlist.service.ts` (ADR-3: pure business logic, no auth — the route/callers do auth/pass `userId`). `saveWeights` validates the direct-percent contract as of ADR-22 (complete group of 5, each in `[0,100]`, `sumsTo100`). Also exports `getWeightsForSettings` (ADR-22) — normalizes then converts to whole percents summing to 100; used only by the settings route's GET, never by the scoring consumers above (which keep calling `getWeights`) |
| `lib/utils/scoring-weights-settings-gate.ts` | Pure `computeGroupTotalState`/`toNumbers` (ADR-22) — the Settings page's per-section running-total, sum-to-100 validity, dirty state, and Save-button gate, extracted out of `app/(dashboard)/settings/page.tsx` because a Next.js App Router `page.tsx` may only export `default`/reserved route-config names; also gives this logic a React-free unit-test seam (`@testing-library/react` is not installed in this repo) |
