...
| `lib/utils/positions-tab.ts` | Pure `shouldShowPositionsTab(transactions)` (has-or-had-a-position tab-visibility gate) and `getPositionsPanelState(position)` (`"held"`/`"closed"`/`"none"` — gates the Positions tab's live stat band on `quantity > 0` specifically, not on position-record presence) shared by `research/[symbol]/page.tsx`, `portfolio/[ticker]/page.tsx`, and `transactions-tab.tsx` (ADR-18) |
