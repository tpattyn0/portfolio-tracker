...
Requires owner decision: SCW-Q1 (resolved 2026-07-21 — migration applied), SCW-Q2 (resolved 2026-07-21 — settings page validation verified live, owner-accepted)
...
### SCW-Q2 — QUESTION — RESOLVED (owner-accepted 2026-07-21)
**Resolution:** Verified live on `/settings` (2026-07-21, on the merged direct-percent revision — see `reviews/2026-07-21-scoring-weights-direct-percent.md`): the page loads both Composite and Fundamental sections with the user's saved percentages; the live group-total indicator reads "TOTAL: 100% · VALID" when valid and switches to "TOTAL: 130% · MUST EQUAL 100%" (red / `--dn`) when a group doesn't sum to 100; Save is disabled while a group is invalid and re-enables when the group returns to a valid 100% total and is dirty; the two groups validate independently. Save-persistence and the on-research-tab composite/kicker propagation were left for the owner to confirm directly (a live mutation of the owner's real preferences, not driven in the walkthrough); owner accepted. Note the "Your weighting" meta-kicker now shows only for genuinely-custom weights per the DP-I1 fix in the direct-percent revision.
