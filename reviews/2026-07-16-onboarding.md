...
Date: 2026-07-16
Status: PARTIALLY IMPLEMENTED — 2026-07-17 (ONB-02 through ONB-09 done; ONB-01 **partially resolved** — NEXTAUTH_SECRET/GEMINI_API_KEY/DATABASE_URL rotated 2026-07-17, NEWS_API_KEY still live and public with risk accepted by the owner conditional on this app not reaching production, see TECH_DEBT.md TD-01 and DECISIONS.md ADR-7; ONB-10/ONB-11 resolved by owner decision, see ADR-6 and TD-02)
...
**Owner decision (2026-07-17) — PARTIALLY ACTIONED.** `NEXTAUTH_SECRET` and `GEMINI_API_KEY` rotated, plus `DATABASE_URL` (already rotated earlier); leaked values verified dead by hashing the current `.env` against `3855042e:.env`. `NEWS_API_KEY` **not rotated** — newsapi.org's free tier exposes no regenerate/revoke control. Owner accepted the residual risk **conditional on this app not being in production**; the key can be spent by anyone but grants no access to our data or users. Recorded as ADR-7 with the non-production condition as a blocking precondition on TD-01. Note the audit's own gap: this finding named only `.env`, but `.env.local` carries the same three keys and is committed in the same two commits — see TD-29 (resolved 2026-07-17: `.env.local` synced to the rotated values; the ID was TD-07 when this note was first written, renumbered to fix a collision).
