...
- **`lib/services/realized-pl.service.ts`** (added 2026-07-17, AUD-03) — single source of truth for FIFO lot matching and realized P/L (including buy and sell fees). Used by both the sell route and the closed-positions route. `matchFifoLots` mutates its `lots` array in place — pass a fresh array per call.
